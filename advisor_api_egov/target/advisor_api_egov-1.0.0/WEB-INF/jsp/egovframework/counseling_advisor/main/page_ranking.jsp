    ...
    .rangking_data_table li div {
        ...
        text-align: center;
				...
				<div class="rangking_title">
						...
						<div class="btn_close">
							<a href="javascript:fnPageLoad('${path}/page/spa.do','');">
			    	  ...
			    	  dataType: 'json',
			   		 success: function(data) {
				          ...
				          var jsonString = JSON.stringify(data);
		            		...
		            		ulElement.html("<li class='header'><div>핸재 순위</div><div>키워드</div><div>상승률</div></li>");
		            		
		            		
				            for (var i = 0; i < data.returnObject[formattedDate.replace(/-/g, '.')].length; i++) {
				                var term = data.returnObject[formattedDate.replace(/-/g, '.')][i].term; // "term" 속성 가져오기
				                var rank = data.returnObject[formattedDate.replace(/-/g, '.')][i].rank; // "term" 속성 가져오기
				                var upSize = 0;
				                	
				                	if(data.returnObject[formattedDate.replace(/-/g, '.')][i].beforeRank==0)
				                	{
				                	...
				                	{
				                		upSize = data.returnObject[formattedDate.replace(/-/g, '.')][i].beforeRank - data.returnObject["2023.09.22"][i].rank; // "term" 속성 가져오기
				                		
				                	}
				                	
				                	
				                    var newLi = $("<li class='rangking_data'><div>" + rank + "</div><div>" + term + "</div><div>" + upSize + "</div></li>");
		    ...
		    success: function(data) {
          		...
          		ulElement.html("<li class='header'><div>순번</div><div>키워드</div><div>건수</div></li>");
		            ...
		            for (var i = 0; i < data.returnObject[formattedDate.replace(/-/g, '.')].length; i++) {
		                var rank = data.returnObject[formattedDate.replace(/-/g, '.')][i].rank; // "term" 속성 가져오기
		                var term = data.returnObject[formattedDate.replace(/-/g, '.')][i].term; // "term" 속성 가져오기
		                var weight = data.returnObject[formattedDate.replace(/-/g, '.')][i].weight; // "term" 속성 가져오기
		            
		                	
		                    var newLi = $("<li class='rangking_data'><div>" + rank + "</div><div>" + term + "</div><div>" + weight + "</div></li>");
		                    ulElement.append(newLi);
