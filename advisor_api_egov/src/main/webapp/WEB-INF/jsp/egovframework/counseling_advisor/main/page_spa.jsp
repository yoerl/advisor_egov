...
<head>
	...
	<script src="<c:url value='/js/egovframework/UploadAdapter.js' />"></script>
</head>
...
<!--로그아웃 팝업창 -->
	<div id="logout_alert_popup" class="alert_popup">
		...
		<div class="del_alert_btn">
			...
			<a href="#" class="bnt_cancle" id=btn_logout_cncl>취소</a><a href="javascript:window.close();" id="btn_logout_confirm">로그아웃</a>
...
<!--소속그룹 변경 저장알림팝업창 -->
<div id="chg_font_popup" class="alert_popup" style="height:125px">
	<div class="del_alert_con">
		<p>폰트를 변경하였습니다.</p>
	</div>
	<div class="del_alert_btn">
		<a href="#" class="bnt_cancle" id="btn_chg_font_cncl">닫기</a>
	</div>
</div>

<!--소속그룹 변경 저장알림팝업창 -->
<div id="chg_fontsize_popup" class="alert_popup" style="height:125px">
	<div class="del_alert_con">
		<p>폰트사이즈를 변경하였습니다.</p>
	</div>
	<div class="del_alert_btn">
		<a href="#" class="bnt_cancle" id="btn_chg_fontsize_cncl">닫기</a>
	</div>
</div>
...
<!--권한 승인팝업창 -->
	<div id="chg_auth_popup" class="alert_popup" style="height:125px">
		<div class="del_alert_con">
			<p>권한을 승인하였습니다.</p>
		</div>
		<div class="del_alert_btn">
			<a href="#" class="bnt_cancle" id="btn_chg_auth_popup_cncl">닫기</a>
		</div>
	</div>
	
	
<!--공지사항 수정 팝업창 -->
	<div id="chg_notice_popup" class="alert_popup" style="height:125px">
		<div class="del_alert_con">
			<p>수정 완료 하였습니다.</p>
		</div>
		<div class="del_alert_btn">
			<a href="#" class="bnt_cancle" id="btn_chg_notice_cncl">닫기</a>
		</div>
	</div>


	<div id="no_chang_news_read_popup" class="alert_popup" style="height:125px">
		<div class="del_alert_con">
			<p>읽음처리 할 항목을 선택해주세요.</p>
		</div>
		<div class="del_alert_btn">
			<a href="#" class="bnt_cancle" id="btn_no_chang_news_read_cncl">닫기</a>
		</div>
	</div>
	...
	<div id="no_quiry_popup" class="alert_popup" style="height:125px">
		<div class="del_alert_con">
			<p>상담중일때만 수동질의가 가능합니다.</p>
		</div>
		<div class="del_alert_btn">
			<a href="#" class="bnt_cancle" id="btn_no_quiry_cncl">닫기</a>
		</div>
	</div>
	
	
	
	<div id="save_councelling_popup" class="alert_popup" style="height:125px">
		<div class="del_alert_con">
			<p>상담요약을 저장 완료 하였습니다.</p>
		</div>
		<div class="del_alert_btn">
			<a href="#" class="bnt_cancle" id="btn_save_councelling_cncl">닫기</a>
		</div>
	</div>
<!-- 팝업 구간 -->
...
//그룹설정 저장 팝업 닫기
$(document).on("click", "#btn_chg_font_cncl", function() {
	$('#chg_font_popup').css('display', 'none');	
});


//그룹설정 저장 팝업 닫기
$(document).on("click", "#btn_chg_fontsize_cncl", function() {
	$('#chg_fontsize_popup').css('display', 'none');	
});

//그룹설정 저장 팝업 닫기
$(document).on("click", "#btn_chg_auth_popup_cncl", function() {
	$('#chg_auth_popup').css('display', 'none');	
});
...
$('#btn_chg_notice_cncl').click(function () {
	$('#chg_notice_popup').css('display', 'none');	
})
$('#btn_no_chang_news_read_cncl').click(function () {
	$('#no_chang_news_read_popup').css('display', 'none');	
})

$('#btn_no_quiry_cncl').click(function () {
	$('#no_quiry_popup').css('display', 'none');	
})


$('#btn_save_councelling_cncl').click(function () {
	$('#save_councelling_popup').css('display', 'none');	
})
...
$(document).on("click", "#btn_ntc_del", function() {
    ...
    // AJAX DELETE 요청 보내기
    $.ajax({
        url: "${path}/api/notice/" + $("#noti_seq").val() + ".do", // 엔드포인트 URL
	        ...
	        stomp.subscribe('/topic/script', function (event) {
               ...
               if(messageData.agentId=='<%=user_id%>'){
			            ...
			            if (messageType === 0) {
			                ...
			                //444444
			                console.log("aaaaaaaaa3333333333333333331111111111111");
			                console.log("통화시작식 페이지 네이션 초기화");
							//통화 시작시 답변영역 초기화
...
/* 			            	pagination_answer = new tui.Pagination('pagination_answer', {
				            ...
				            if($("#chating_popup").css('display')==='none'&&messageData.rxTxFlag!=="MX")
				            {
				                ...
				                // 삽입할 HTML 코드를 생성합니다.
									var insertedHTML = '<div class="chating_head_inner">' +
									                  '<h2>'+messageData.orgPhoneNumber+'고객님과 전화상담이 시작되었습니다.</h2>' +
									                  '<p>시작일시 ('+messageData.segmentOffset+')</p>' +
									                  '</div>';
			                ...
			                $('.chating_con').css('height', 'calc(100% - 90px)');
							...
							var insertedHTML = ""
								...
								else
								{
									insertedHTML += '<span class="dengerus"><i>!</i>' + messageData.blackList + '</span>';
								      ...
								      var jsonData_news = {
								            	"receiverId": "<%=user_id%>" ,
								            	"newsTitlNm": "위험 키워드가 감지 되었습니다. 위험 키워드 - " + messageData.blackList,
								            	"newsCntn": "위험 키워드 - " + messageData.blackList
											}
			            	...
			            	// 요약내용 삽입합니다.
			            	$('#summary_text').val(messageData.summary + "에 대한 상담입니다.");
			            ...
			            //$('#chating_popup').css('display', 'block');
			            //$('.chating_con').css('height', 'calc(100% - 240px)');
			            
			            
			            //$("#chating_contents").val(messageData.callId);
			            
			            if($("#chating_popup").css('display')==='block')
			            {
			            	$('.chating_con').css('height', 'calc(100% - 240px)');
			            	//$("#call_id").val("");
			            }
	        ...
	        stomp.subscribe('/topic/answer', function (event) {
	            ...
	            {
					console.log("dsvwqewwwwwwwaaaaaaa");
					
					console.log(messageData.searchType!=="ADA001B004");
					console.log("as " + "messageData.searchType");
					
					//자동 답변이 아닌경우 = 수동답변
					if(messageData.searchType!=="ADA001B004")
					{				
						var insertedHTML = `
			                <div class="counsel_date"><p>`+formattedDateTime+`</p></div>
						    ...
						    insertedHTML += `<li class="ai_part">
				        		          ...
				        		          <div>
				        		            	...
				        		            	<h4>` + $("#text_adv_menual_question").val() + `</h4>
								        		  </div>
								        	 </div>
								        </li> 
								  ...
								  <li class="ai_part">
								    ...
								    <div>
								      <div class="ai_part_txt">
								        <h4>답변을 찾을 수 없습니다. 다시 질문 해 주세요.</h4>
								      </div>
								    </div>
								  </li>
								</ul>
	            ...
	            else if(messageData.agentId=='<%=user_id%>')
	            {
	            	...
	            	//1111111
      	            ...
      	            //pagination1.setTotalItems(data.totalResultCount);
	            	//pagination_answer
	            	
	            	console.log("aaaaaaaaaaa1111111111111111111111111111111111111111111111111");
	            	
	            	

	            	//pagination_answer.setTotalItems(data.totalResultCount);
	            	//pagination_answer.setTotalItems(("#answer_cnt").val());
          ...
          insertedHTML += '      </div>' +
		        		  ...
		        		  insertedHTML += '    <li class="ai_part">' +
		        		            	  ...
		        		            	  if(messageData.AI.question !== null && messageData.AI.question !== undefined)
		        		            	  {
		        		            			insertedHTML += '          <h4>' + messageData.AI.question + '</h4>';
		        		            			question = messageData.AI.question.replace(/"/g, "'");
					        		         	  
		        		            	  }
		        		            	  else
		        		            	  {		
		        		            		  insertedHTML += '          <h4>' + $("#text_adv_menual_question").val() + '</h4>';
		        		            			question =$("#text_adv_menual_question").val().replace(/"/g, "'");
		        		  ...
		        		  insertedHTML += '        </div>' +
		        		            	  '      </div>' +
		        		            	  '    </li>';
          if(messageData.STATUS_MSG!=="ai_404" ||messageData.AI.answer !== null && messageData.AI.answer !== undefined){
        	  var answer = messageData.AI.answer.replace(/"/g, "'");
		  ...
		  insertedHTML += '    <li class="ai_part">' +
		            	  ...
		            	  '            <span><button class="chang_open_btn trigger"><input type="hidden" class="counsel_poopup_title_val" value="'+question+'"><input type="hidden" class="counsel_poopup_con_val" value="'+messageData.AI.answer+'"></button></span>' +
		            	  '            <span><input type="checkbox" data-key="'+messageData.AI.key+'" id="ai_check_' + $("#answer_cnt").val() + '" name="ai_check_' + $("#answer_cnt").val() + '"><label for="ai_check_' + $("#answer_cnt").val() + '"></label></span>' +
		            	  '          </div>' +
		            	  '        </div>' +
		            	  '      </div>' +
		            	  '    </li>';
		  ...
		  insertedHTML += '    <li class="kms_part">' +
		            	...
		            	for (var i = 0; i < messageData.KMS.length; i++) {
		            	  ...
		            	  insertedHTML += '            <li><a href="javascript:window.open(\''+messageData.KMS[i].popupUrl+'\', \'_blank\', \'width=1220, height=985\');"><div class="kms_part_con"><p>' + messageData.KMS[i].title + '</p>' +
		                  '              <span><input type="checkbox" data-key="'+messageData.KMS[i].key+'" data-cntnt-id="'+messageData.KMS[i].cntntId+'" id="kms_check_' + $("#answer_cnt").val()+"_"+(i+1)+ '" name="kms_check_' + $("#answer_cnt").val()+"_"+(i+1) + '"><label for="kms_check_' + $("#answer_cnt").val()+"_"+(i+1) + '"></label></span>' +
		                  '            </div></a></li>';
		...
		$("#menu_logout").click(function(){ 
			...
			$(document).on("click", "#btn_save_summary", function() {
	            ...
	            // 서버로 보낼 JSON 데이터
					...
					console.log("1111111111111111111111111111111122" + jsonData_kms);
					console.log("1111111111111111111111111111111122" + JSON.stringify(jsonData_kms));
					console.log("11111111111111111111111111111111333");
		            // AP 서버에 전송
					  $.ajax({
					      type: "POST",
					      ...
					      success: function(data) {
	            			  ...
	            			  $('#save_councelling_popup').css('display', 'block');	
					  ...
					  $('[id^="kms_check_"]:checked').each(function() {
					    ...
					    checkboxIDs.push(dataCntntId);
		    		   	...
		    		   	console.log("kms에 저장 던지기1111");
		    		   	console.log("kms에 저장 던지기2222" + jsonData_recommend);
		    		   	console.log("kms에 저장 던지기3333" + JSON.stringify(jsonData_recommend));
					  ...
					  $('[id^="kms_check_"]:checked').each(function() {
					    ...
					    checkboxKMSkey.push(dataKey);
					  });
					  var checkAikey = [];
					  $('[id^="ai_check_"]:checked').each(function() {
					    var dataKey = $(this).attr('data-key');
					    
					    checkAikey.push(dataKey);
					  });
					  
					  
		    		   	var jsonData_recommend_local = {
		    			            		  "userId": "<%=user_id%>",
		    			            		  "kmsKey":  checkboxKMSkey,
		    			            		  "aiKey":  checkAikey
		    							}
	    			  ...
	    			  $.ajax({
	    			      type: "POST",
	    	              url: "${path}/api/recommend.do", // 엔드포인트 URL
	    	              data: JSON.stringify(jsonData_recommend_local), // JSON 데이터 문자열로 변환
	    	              contentType: "application/json", // 요청 본문의 데이터 타입 설정
	    	              async:false,
	    	              success: function(data) {
	    			          var jsonString = JSON.stringify(data);
							  ...
							  console.log("k5345지55553 : "+ data);
							  console.log("km2342425553 : "+ jsonString);
							  console.log("kms에 346326553");
	    			      },
	    			      error: function(request, status, error) {

	    			    	  console.log("k123123장 ㅎ3553");
	...
	<input type="hidden" id ="answer_cnt" value="0">
...
function fn_ai_quiry_answer() {

	console.log("33333333333333333333 : " + $("#call_id").val());
	if($("#call_id").val()=="")
	{
		//alert("상담중일때만 수동질의가 가능합니다.");
		

		$('#no_quiry_popup').css('display', 'block');
	}
	else
	{
        $.ajax({
            ...
            success: function(response) {
                ...
                var jsonData = {
		          ...
		          console.log("fsfsfs3141f " + jsonData);
		          console.log("fsfsfs123123f " + JSON.stringify(jsonData, null, 2));
